...
def get_model(debugger, models_name, targets_type, prediction_type, number_of_classes, parameters):
    if (models_name == CONVO_CARPET_NAME):
        if targets_type == SINGLE_TARGET:
            return ConvoCarpetSingleTarget(debugger, prediction_type, number_of_classes, kernels_count=parameters[KERNELS_COUNTS], sentences_count=parameters[SENTENCES_COUTS], hidden_layer_1=parameters[HIDDEN_LAYER1])
        else: 
            return ConvoCarpetMultiTarget(debugger, prediction_type, number_of_classes, kernels_count=parameters[KERNELS_COUNTS], sentences_count=parameters[SENTENCES_COUTS], hidden_layer_1=parameters[HIDDEN_LAYER1])
